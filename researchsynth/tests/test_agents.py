...
from researchsynth.state.schemas import CycleState, ExtractedFinding, PaperRecord, ContradictionPair
from researchsynth.agents.synthesis_writer import SynthesisWriter
...
def test_synthesis_writer_valid(base_state):
    ...
    with patch("researchsynth.agents.synthesis_writer.generate_json", return_value=mock_response) as mock_llm:
...
def test_synthesis_writer_invalid_finding_id(base_state):
    ...
    with patch("researchsynth.agents.synthesis_writer.generate_json", return_value=mock_response):
...
def test_synthesis_writer_no_findings():
    ...
    writer = SynthesisWriter()
    with patch("researchsynth.agents.synthesis_writer.generate_json") as mock_llm:
...
def test_synthesis_writer_empty_response(base_state):
    ...
    writer = SynthesisWriter()
    with patch("researchsynth.agents.synthesis_writer.generate_json", return_value={"markdown_report": "   "}):
...
def test_synthesis_writer_llm_exception(base_state):
    ...
    writer = SynthesisWriter()
    with patch("researchsynth.agents.synthesis_writer.generate_json", side_effect=Exception("API Error")):
...
def test_synthesis_writer_valid_citations_after_punctuation(base_state):
    ...
    for resp in mock_responses:
        with patch("researchsynth.agents.synthesis_writer.generate_json", return_value={"markdown_report": resp}):
